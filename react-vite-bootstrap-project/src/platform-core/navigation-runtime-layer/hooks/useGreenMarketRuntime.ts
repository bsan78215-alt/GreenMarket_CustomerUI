...
/** Прямой доступ к экземпляру Runtime (включая forceNavigate) — предназначен
 *  ИСКЛЮЧИТЕЛЬНО для моста Runtime↔React Router на уровне App Shell
 *  (src/app/RuntimeRouteSync.tsx). Экраны должны использовать
 *  useGreenMarketRuntime()/useCurrentScreen()/useBusinessEvents(), а не этот
 *  хук — он не проверяет availableActions и обходит dispatch(). */
export function useRuntimeInstance(): GreenMarketRuntime {
  return useRuntime();
}
